    ...
    <v-card class="mb-3">
     ...
     <v-card>
        ...
        <template v-slot:item.cause="{ item }">
            <span v-html="item.cause"></span>
        </template>

        <template v-slot:item.solution="{ item }">
            <span v-html="item.solution"></span>
        </template>
...
export default {
    ...
    methods: {
        onRecovery(id) {
            ...
            this.axios.get('http://localhost:8000/api/crop-disease/'+ id + '/recovery', {
                headers: {
        ...
        onForceDelete(id) {
            const token =  window.localStorage.getItem('accessToken');
            this.axios.delete('http://localhost:8000/api/crop-disease/'+ id + '/forceDelete', {
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
                    'Authorization': 'Bearer ' + token
        ...
        fetchData() {
            const token =  window.localStorage.getItem('accessToken');
            this.axios.get('http://localhost:8000/api/crop-disease/trash', {
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
                    'Authorization': 'Bearer ' + token
